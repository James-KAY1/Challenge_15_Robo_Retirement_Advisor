...
def validate_age_investment(age, investment_amount):
    ...
    if age is not None:
        age = parse_int(age)
        if age < 0: 
            return build_validation_result(False, "age", "Your age needs to be positive")
        elif age >= 65:
            return build_validation_result(False, "age", "Age is above plan requirements, please input an age under 65.")
            
    if investment_amount is not None:
        investment_amount = parse_int(investment_amount)
        if investment_amount < 5000:
            return build_validation_result(False, "investmentAmount", "The amount must be greater than $5000")
